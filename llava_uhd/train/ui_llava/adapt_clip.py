...
class adapt_CLIPVisionTower(nn.Module):
    ...
    def train(self, trainable=True):
        ...
        self.vision_tower.vision_model.embeddings.requires_grad_(False)
    ...
    def forward(self, images, origin_image_widths,origin_image_heights):
        ...
        split_images = torch.chunk(images, chunks=images.shape[1] // 3, dim=1) # B x [3*(#slices + 1)] x 336 x 336 -> B x 3 x 336 x 336 each in the resulting list
        slice_w_nums=[]
        slice_h_nums=[]
        abstract_w_nums=[]
        abstract_h_nums=[]
